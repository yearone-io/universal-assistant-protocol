...
describe("UniversalReceiverDelegateUAP", function () {
  ...
  describe("Executive and Screener Combinations", function () {
    ...
    it("should handle multiple screeners with NO EXECUTION when ONE IS FALSE on AND CHAIN for TipAssistant", async function () {
      ...
      const typeKey = generateMappingKey("UAPTypeConfig", LSP0_TYPEID);
      await universalProfile.setData(typeKey, customEncodeAddresses([tipAssistantAddress]));
      await setScreenerConfig(universalProfile, tipAssistantAddress, [trueScreenerAddress, falseScreenerAddress, trueScreenerAddress], LSP0_TYPEID, ["0x","0x","0x"], true);
      await setExecutiveConfig(universalProfile, tipAssistantAddress, ethers.AbiCoder.defaultAbiCoder().encode(["address", "uint256"], [await nonOwner.getAddress(), 10]));
    ...
    it("should handle multiple screeners with EXECUTION when ALL TRUE on AND CHAIN for TipAssistant", async function () {
      ...
      const typeKey = generateMappingKey("UAPTypeConfig", LSP0_TYPEID);
      await universalProfile.setData(typeKey, customEncodeAddresses([tipAssistantAddress]));
      await setScreenerConfig(universalProfile, tipAssistantAddress, [trueScreenerAddress, trueScreenerAddress, trueScreenerAddress], LSP0_TYPEID, ["0x","0x","0x"], true);
      await setExecutiveConfig(universalProfile, tipAssistantAddress, ethers.AbiCoder.defaultAbiCoder().encode(["address", "uint256"], [await nonOwner.getAddress(), 10]));
      const initialBalanceUP = await ethers.provider.getBalance(mockUPAddress);
      const initialBalanceNonOwner = await ethers.provider.getBalance(await nonOwner.getAddress());
      await owner.sendTransaction({ to: mockUPAddress, value: ethers.parseEther("1") });
    ...
    it("should handle multiple screeners with NO EXECUTION when ALL ARE FALSE on OR CHAIN for TipAssistant", async function () {
      ...
      const typeKey = generateMappingKey("UAPTypeConfig", LSP0_TYPEID);
      await universalProfile.setData(typeKey, customEncodeAddresses([tipAssistantAddress]));
      await setScreenerConfig(universalProfile, tipAssistantAddress, [falseScreenerAddress, falseScreenerAddress, falseScreenerAddress], LSP0_TYPEID, ["0x","0x","0x"], false);
      await setExecutiveConfig(universalProfile, tipAssistantAddress, ethers.AbiCoder.defaultAbiCoder().encode(["address", "uint256"], [await nonOwner.getAddress(), 10]));
      const initialBalanceUP = await ethers.provider.getBalance(mockUPAddress);
      const initialBalanceNonOwner = await ethers.provider.getBalance(await nonOwner.getAddress());
      await owner.sendTransaction({ to: mockUPAddress, value: ethers.parseEther("1") });
